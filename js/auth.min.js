...
if(loginRedirect.includes("/bot")) window.location.href = location.href.replace('/auth','/bot')

// === Params â†’ objeto simples ===
const params = {};
urlParams.forEach((value, key) => { params[key] = value; });

// DetecÃ§Ã£o robusta de Ã­ndices (N) existentes: acctN/tokenN/curN
...
// Helper: pega trio por N (garante coerÃªncia)
...
function connectWebSocket() {
  ...
  ws.onopen = () => {
    ...
    // Autoriza com o primeiro token vÃ¡lido da lista (ordem dos Ã­ndices)
    const first = indices.map(getTriple).find(x => !!x.token);
    if (!first) {
      console.error('Nenhum token* nos parÃ¢metros.');
  ...
  ws.onmessage = (ev) => {
    ...
    if (data.msg_type === 'authorize') {
      if (data.authorize) handleAuthorization(data.authorize);
        ...
        alert(data.error.message || 'Erro de autorizaÃ§Ã£o.');
        redirectToBot();
      }
    }
  };
...
function handleAuthorization(auth) {
  ...
  // 2) Descobrir N cujo acctN === auth.loginid â†’ daÃ­ vem o activeToken correto
  ...
  // 3) Montar tokenList preservando N: acctN â†” tokenN â†” curN
  //    Enriquecemos loginInfo com dados do authorize/account_list quando possÃ­vel.
  ...
  const tokenList = indices
    .map(n => {
      ...
      // Base mÃ­nima (fallback) se nÃ£o houver no account_list
      const fallbackInfo = {
        loginid,
      ...
      // Se Ã© o login autorizado, usar o objeto completo de auth (traz balance etc.)
      const loginInfo =
      ...
      return {
        accountName: loginid,         // vocÃª pode trocar por um label diferente se quiser
        token: token,
...
// Iniciar
connectWebSocket();
// Se seu fluxo precisa forÃ§ar reload (cautela):
